...
class Chat2Activity : ComponentActivity() {
        ...
        setContent {
            SOSTheme {
                Surface(
                    ...
                    ChatScreen2(userId = userId, receiverId = receiverId, nickname = nickname, socket = socket)
                }
            }
        }
    }
    ...
    companion object {
        fun newIntent2(context: Context, userId: Int, receiverId: Int, nickname: String): Intent {
            return Intent(context, Chat2Activity::class.java).apply {
...
@Composable
fun ChatScreen2(userId: Int, receiverId: Int, nickname: String, socket: Socket) {
        ...
        Column(modifier = Modifier
            ...
            LazyColumn(
                ...
                items(messages) { message ->
                    MessageItem2(message = message, isOwnMessage = message.status == 2)
                }
            }
...
@Composable
fun MessageItem2(message: Message, isOwnMessage: Boolean) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp),
